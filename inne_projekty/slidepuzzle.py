  # Slide Puzzle - by Al Sweigart from inventwithpytom.com
...
YMARGIN = int((WINDOWHEIGHT - (TILESIZE * BOARDHEIGHT + (BOARDHEIGHT - 
              1))) / 2)
UP = 'up'
DOWN = 'down'
LEFT = 'left'
...
def main():
    ...
    SOLVEDBOARD = getStartingBoard() # a solved board the same as the starting
    ...
    while True:  # main game loop
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        mainBoard, solutionSeq = generateNewPuzzle(80) #cli-
                                                        #cked on Game button
                        allMoves = []
                ...
                if event.key in (K_LEFT, K_a) and isValidMove(mainBoard, 
                                LEFT):
                    slideTo = LEFT
                elif event.key in (K_RIGT, K_d) and isValidMove(mainBoard, 
                                  RIGHT):
                    slideTo = RIGHT
        ...
        if slideTo:
            slideAnimation(mainBoard, slideTo, 'Click tile or press arrow key \
...
def makeMove(board, move):
    ...
    if move == UP:
        board[blankx][blanky], board[blankx][blanky + 1] = board[
                blankx][blanky +1], board[blankx][blanky]
    elif move == DOWN:
        board[blankx][blanky], board[blankx][blanky - 1] = board[
                blankx][blanky - 1], board[blankx][blanky]
    elif move == LEFT:
        board[blankx][blanky], board[blankx + 1][blanky] = board[
                blankx + 1][blanky], board[blankx][blanky]
    elif move == RIGHT:
        board[blankx][blanky], board[blankx - 1][blanky] = board[
                blankx - 1][blanky], board[blankx][blanky]
...
def slideAnimation(board, direction, message, animationSpeed):
    ...
    if direction == UP:
        ...
        # draw a blank space over the moving tile on the baseSurft Surface.
